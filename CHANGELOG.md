...
- LLM structured-output recovery now also holds for **F2**: the F2 merge-decider schema merged `targetConceptId` (UUID string in three branches, `null` in `unrelated`) and `resultStatus` (enum vs literal) across its discriminated-union branches into a type-less `{}` property node, which OpenAI-family `response_format` validation rejects (HTTP 400, "schema must have a 'type' key") — failing every F2 merge-decider call on OpenAI/OpenRouter. The provider-schema normalizer now always emits a legal `type` (or JSON-Schema type array) per property, pinned by regression tests for both provider families.
